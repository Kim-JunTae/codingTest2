...
//        String s = "aaab";
    ...
    public static String reorganizeString(String s) {
        ...
        while (queue.size() > 1){
            //가장 갯수가 많은 문자부터 시작
            Character first = queue.poll();                 //가장 갯수가 많은 문자
            reorganizedString.append(first);                //추가
            charIntMap.put(first, charIntMap.get(first)-1); //맵에서 갯수 변경

            //두번째로 갯수가 많은 문자 시작
            Character second = queue.poll();
            reorganizedString.append(second);
            charIntMap.put(second, charIntMap.get(second)-1);

            if(charIntMap.get(first) > 0) queue.add(first);
            if(charIntMap.get(second) > 0) queue.add(second);
        }
